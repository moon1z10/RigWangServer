...
class TeamRedMiner {
    ...
    update = (rig, ip = this.ip, port = this.port, d = this.d, curDateTime = this.curDateTime, gpuArr = this.gpuArr) => new Promise((resolve, reject) => {
        ...
        socket.on('data', function (chunk) {
            resJsonStr += chunk.toString();
            console.log(`TeamRedMiner(${ip}:${port}) socket.on('data -> chunk' : ${chunk.toString()})`);
        });
        socket.on('end', function () {
            try {
                var minerData = JSON.parse(resJsonStr);
                ...
                console.error('TeamRedMiner parsing error/resJsonStr : ' + resJsonStr);
                reject(e);
            }
